...
const ContractModal1 = observer(({ setVisible, setLoading }) => {
  ...
  function checkChange() {
    if (!check.current.checked || serNumberError || dirNumberError || fullNameError) {
      setFormValid(false);
    } else {
      setFormValid(true);
    }
  ...
  function createAndDownloadPdf() {
    setLoading(true);
    axios
      .post("http://172.20.0.49:5000/create-pdf1", server)
      .then(() => 
        axios.get("http://172.20.0.49:5000/contract1", { responseType: "blob" })
      )
      .then((res) => {
