...
const ContractModal3 = observer(({ setVisible, setLoading }) => {
  ...
  const [pdf, setPdf] = useState({
    ...
    address: `__________________________
              ...
              __________________________`,
    mobilePhone: "__________________________",
    homePhone: "__________________________",
    organName:
      "___________________________________________________________________________________",
    organAddress: `___________________________
  ...
  function createAndDownloadPdf() {
    setLoading(true);
    axios
      .post("http://localhost:5000/create-pdf3", server)
      .then(() =>
        axios.get("http://localhost:5000/contract3", { responseType: "blob" })
      )
      .then((res) => {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex" }}>
        <input id="check3" type="checkbox" ref={check} onChange={checkChange} />
        <label
          style={{
            marginLeft: "10px",
            marginTop: "-5px",
